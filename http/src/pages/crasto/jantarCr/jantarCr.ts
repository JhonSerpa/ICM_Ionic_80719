...
import { CrastoPage } from '../crasto';

@Component({
    selector: 'page-jantarCr',
    templateUrl: 'jantarCr.html'
  })
  export class JantarCrPage {
            ...
            .subscribe(text => {
                ...
                this._robotOverlords= text;
                    for (var i = 0; i< text.menus.menu.length; i++){
                        try{
                            if(text.menus.menu[i]['@attributes'].meal == "Jantar" && text.menus.menu[i]['@attributes'].canteen == "Refeitório do Crasto"){
